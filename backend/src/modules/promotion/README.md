# promotion — Promotions bounded context

**Not implemented yet.** The hexagon is scaffolded so the shape is obvious; no code lives
here.

Scope: Promotion. Discount rules, validity windows, applicability to an order.
...
5. Bind port → adapter in `promotion.module.ts` and import it from `app.module.ts`.
