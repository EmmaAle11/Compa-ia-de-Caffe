# payment — Payments & Billing bounded context

**Not implemented yet.** The hexagon is scaffolded so the shape is obvious; no code lives
here.

Scope: Payment, the idempotency ledger, Stripe Checkout sessions and webhook verification.
...
5. Bind port → adapter in `payment.module.ts` and import it from `app.module.ts`.
