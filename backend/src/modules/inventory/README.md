# inventory — Inventory & Fulfillment bounded context

**Not implemented yet.** The hexagon is scaffolded so the shape is obvious; no code lives
here.

Scope: Inventory. 15-minute checkout reservations, released when the session expires unpaid.
...
5. Bind port → adapter in `inventory.module.ts` and import it from `app.module.ts`.
